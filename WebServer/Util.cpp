...
// 如果尝试send到一个disconnected socket上 底层会抛出一个SIGPIPE信号
// 这个信号的缺省处理方法是退出进程 大多数时候这都不是我们期望的
// 因此我们需要重载这个信号的处理方法 比如忽略
void handleForSigpipe() {
    struct sigaction sa;
    memset(&sa, 0, sizeof(sa));
    sa.sa_handler = SIG_IGN;  // ignore singal
    sa.sa_flags = 0;
    if (sigaction(SIGPIPE, &sa, NULL)) return;
}

// ref "Server-side Development: Techonology and Practices" p.251
...
void setSocketNoDelay(int fd) {
    int enable = 1;
    setsockopt(fd, IPPROTO_TCP, TCP_NODELAY, static_cast<const void *>(&enable), sizeof(enable));
}
...
void setSocketNoLinger(int fd) {
    ...
    linger_.l_onoff = 1;
    linger_.l_linger = 30;
    setsockopt(fd, SOL_SOCKET, SO_LINGER, static_cast<const void *>(&linger_), sizeof(linger_));
}
